...
| UX-013 | 历史记录 | 缺少详情页，只能看摘要 | P0 | addressed | 已新增轻量 `HistoryDetailView`；后续产品优化已收敛为称骨诗文、命格详解和系统分享入口 | 是 | 历史回看价值不足 | V1.5 阶段 6 |
| UX-014 | 历史记录 | 清空全部缺少二次确认 | P0 | addressed | 已为清空全部增加确认弹窗，取消不会删除 | 是 | 误删风险 | V1.5 阶段 6 |
| UX-015 | 历史记录 | 缺少页面级“仅本地保存”说明 | P0 | addressed | V1.5 阶段曾补充历史记录本地保存说明；后续产品优化已移除列表顶部说明卡片，空状态和隐私材料仍说明本地保存边界 | 是 | 隐私说明不够明显 | V1.5 阶段 6 |
| UX-016 | 历史记录 | 删除按钮仅图标，VoiceOver 标签可能不足 | P1 | addressed | 后续产品优化已移除常驻图标删除按钮，单条记录改为 iOS 左滑删除操作 | 是 | 可访问性 | V1.5 阶段 8 |
