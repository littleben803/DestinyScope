...
用户已在 iPhone 真机上完成 V1.2 本地模型 benchmark。
...
- 当前 0.5B Q4 模型在该真机上生成速度可接受。
- 高风险样例成功触发 fallback，安全检查和模板回退链路有效。
- 当前测试只覆盖一台 iPhone，设备型号未细化。
...
建议先在 1 台主力 iPhone 真机上完成 Debug 手工测试，再补 1 台较老 iPhone。阶段 6 完成后再进入 V1.2 阶段 7：是否进入生产路径决策。
