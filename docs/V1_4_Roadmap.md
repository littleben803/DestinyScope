...
- V1.4 阶段 3 已完成：设备 tier 检测技术设计与最小实现。
- V1.4 阶段 4 已完成：模型可用性检测与导入路径整理。
- V1.4 阶段 5 已完成：结果页本地润色预览卡片。
- V1.4 阶段 6 已完成：超时、低电量、过热和失败回退。
- V1.4 阶段 7 已完成：开源许可页面和隐私页面草案落地。
- V1.4 阶段 8 已完成：TestFlight 自测与是否提交内测决策。
...
- 未识别设备不默认启用。
- 有可测试的回退路径。
- 已新增 `DeviceModelIdentifier`、`DeviceTier` 和 `DeviceTierService`。
- `LocalModelExperimentAvailability` 已结合构建、用户确认和设备 tier 判断可用性。
- 实验设置页已展示当前设备 identifier、tier、说明、是否允许和推荐超时。
- `iPhone13,1` / iPhone 12 mini 归入 Tier C，默认禁用。
- Simulator 仅作为 Debug 测试允许，不代表真机性能。
- 下一阶段为 V1.4 阶段 4：模型可用性检测与导入路径整理。
...
- 模型存在时能通过可用性检查。
- 仓库内无模型文件。
- 已新增 `LocalModelFileStatus` 和 `LocalModelFileResolver`。
- 路径优先级为 App Documents、App Documents alias、Developer LocalModels、Developer LocalModels alias。
- `LocalModelDebugConfig` 已复用统一路径解析逻辑，仍兼容原有 `~/LocalModels` 调试路径。
- `LocalModelFileImporter` 会将 `.gguf` 复制到 App Documents 目标路径，且不上传、不下载。
- 实验设置页已展示模型文件状态、来源、大小、可用性和导入入口。
- 下一阶段为 V1.4 阶段 5：结果页本地润色预览卡片。
...
- 点击生成后原始结果仍可见。
- 失败时显示回退文案。
- 已新增 `LocalRefiningPreviewCard`，展示在结果页底部。
- 已新增 `LocalRefiningPreviewInputBuilder` 和 `LocalRefiningPreviewState`。
- 卡片只在构建、用户确认、用户开关、设备 tier 和模型文件均满足时展示。
- 点击按钮后才调用实验 refiner，润色结果只在卡片内临时展示。
- 原始结果页内容不被覆盖，历史记录不写入润色结果。
- 下一阶段为 V1.4 阶段 6：超时、低电量、过热和失败回退。
...
- 安全检查失败回退。
- 低电量 / 过热回退。
- 已新增 `LocalModelRuntimeGuard`、`LocalModelRuntimeState`、`LocalModelRuntimeError`、`LocalModelTimeoutPolicy` 和 `LocalModelFailureTracker`。
- 低电量模式、serious / critical thermal state 会在生成前拦截并回退原始模板文本。
- Tier A / Tier B 使用超时策略，Simulator Debug 使用 5 秒测试超时。
- 连续失败达到阈值后会暂时阻止本地润色。
- UI 层生成超时后显示回退文案；底层 llama.cpp 调用若不可立即取消，后续阶段仍需优化。
- 下一阶段为 V1.4 阶段 7：开源许可页面和隐私页面草案落地。
...
- 隐私说明覆盖本地处理、不上传和回退。
- Release 文案不夸大模型能力。
- 已新增 App 内“开源许可”页面，并在设置页和关于页提供入口。
- App 内隐私政策已补充本地历史记录和本地模型润色实验说明。
- GitHub Pages 隐私页 `docs/privacy/index.html` 和 `docs/privacy/privacy.md` 已同步更新。
- 已新增 `docs/V1_4_LegalUpdateSummary.md`。
- 下一阶段为 V1.4 阶段 8：TestFlight 自测与是否提交内测决策。
...
- 输出 TestFlight 决策。
- 已新增 `docs/V1_4_TestFlightReadinessReport.md`。
- 已新增 `docs/V1_4_TestFlightDecision.md`。
- V1.4 总体结论为 Conditional Go for internal TestFlight preparation。
- App Store 正式发布仍为 No-Go。
- 默认启用本地模型仍为 No-Go。
- 替换默认结果页输出仍为 No-Go。
