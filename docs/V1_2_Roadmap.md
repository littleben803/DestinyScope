...
- 阶段 3A：已完成。已新增 llama.cpp Debug-only 接入骨架和实验入口，但不加载真实模型。
- 阶段 3B：已完成。已新增模型文件准备和 llama.cpp 接入方案文档，仍不下载模型、不接依赖、不改主流程。
- 阶段 3D：工程接入完成。已修复 Codex PATH 中 CMake 不可见的问题，在仓库外构建 `llama.xcframework`，Debug-only 接入工程；Debug / Release 构建通过，macOS smoke test 和 iOS Simulator Debug UI 均已验证同一 GGUF 可加载并生成短文本。
- 阶段 4：已完成。`LocalLlamaTextRefiner.refine(_:)` 已接入 `TextRefining` 抽象，Debug UI 可测试本地模型润色；默认输出仍为 `TemplateTextRefiner`。
- 阶段 5：已完成。已优化 PromptBuilder 和 SafetyChecker，新增 Debug-only 固定安全测试样例和回退策略文档；默认输出仍不受影响。
- 阶段 6：已完成首轮真机 benchmark。0.5B Q4 模型在一台 iPhone / iOS 26.2 上正常样例平均 total 约 `1.09` 秒，高风险样例成功触发 fallback；仍需补充更多设备和 Instruments 数据。
- 阶段 7：已完成。V1.2 总体判定为 PoC 成功，但不直接进入生产路径；当前结论为 Conditional Go 到 V1.3 生产化方案设计。
- 阶段 7B：已完成。已补充 iPhone 17 Pro Max 和 iPhone 12 mini 双设备 benchmark；iPhone 12 mini 可运行但明显较慢，V1.3 需要设备分级。
- 下一阶段：V1.3 本地模型生产化方案设计，第一阶段建议仍只写方案文档，不改生产代码。
...
### 阶段 3A：Debug-only 接入骨架

阶段目标：

- 新增 `LocalLlamaTextRefiner` 占位实现。
- 新增 Debug-only 实验入口。
- 不加载真实模型。
- 不新增 llama.cpp 依赖。
...
```text
Add debug-only local model skeleton
```
...
- 确认首个 GGUF 模型、备选模型和本地文件命名。
- 确认模型本地放置路径和 `.gitignore` 保护。
- 确认 llama.cpp 接入路线和阶段 3C 验收清单。
- 不下载模型、不新增依赖、不改 Swift 代码。

允许修改范围：

- `docs/V1_2_ModelFileSetup.md`
- `docs/V1_2_LlamaCppIntegrationPlan.md`
- `docs/V1_2_LlamaCppPoCGuide.md`
- `docs/V1_2_Roadmap.md`
- `docs/AppStoreChecklist.md`
- `.gitignore`，仅当模型忽略规则不足时
...
- 模型文件准备文档存在。
- llama.cpp 接入路线文档存在。
- `.gitignore` 覆盖 GGUF、bin、safetensors、Core ML 模型和本地模型目录。
- 明确阶段 3C 才做真实加载。

建议 commit message：

```text
Document llama.cpp PoC model setup
```
...
- 如果 no-go，明确继续保留模板方案。
- 已新增 `docs/V1_2_ProductionDecision.md`，结论为 Conditional Go：V1.2 不进入生产，建议进入 V1.3 生产化方案设计。
- V1.2 总体状态：PoC 成功，默认 Release 路径继续使用本地规则和模板输出。
