...
- V1.6 新增的本地数据能力需要重点测试：
  ...
  - 历史记录的本地保存、回看、删除和历史详情分享。
  - 结果页系统分享文案安全边界。
  - 知识库收藏和最近阅读的本地保存与清空。
  - 本地数据管理入口是否能删除历史、收藏、最近阅读、搜索历史和首次使用说明状态。
  - 所有新增数据均不上传、不同步、不登录。
...
V1.6 阶段 3 常用出生资料下线复核项：

- 后续产品优化已下线常用出生资料功能。
- 首页不展示常用资料卡片。
- 首页不展示保存为常用资料入口。
- 设置页不展示常用出生资料管理入口。
- 本地数据管理不展示常用出生资料单独清理项。
- App 内隐私政策和 GitHub Pages 隐私页不再说明常用出生资料。
- Review Notes 和截图计划不得要求审核人员测试常用出生资料。
...
- 默认分享文本不包含完整出生日期、完整农历生日、具体出生时辰或历史记录内部信息。
- 分享文本不包含本地模型润色结果。
...
- 设置页可打开“本地数据管理”。
- 本地数据摘要显示历史记录、知识库收藏、最近阅读和首次使用说明状态。
...
- 已同步 `docs/AppStoreMetadata.md`、`docs/AppReviewNotes.md` 和 `docs/ScreenshotPlan.md` 到 V1.9 App Store 材料口径。
- 当前只准备 App Store 上线材料，不上传 TestFlight，不创建 App Store Connect 记录。
- 本阶段未修改 Swift 代码、Xcode 工程、资源文件、签名、Bundle ID、Team、Version 或 Build。
- 如未来恢复 TestFlight，仍需重新执行 Release 构建、签名、Archive / IPA、测试人员说明、审核备注、隐私说明和包内容复核。

V1.9 阶段 4 最终一致性修复状态：

- 已新增 `docs/V1_9_FinalSubmissionConsistencyReport.md`。
- 当前 App 不再包含常用出生资料用户功能。
- V1.9 最终 App Store / Review / Screenshot / Privacy Nutrition Label 材料已移除常用出生资料和 saved birth profiles 表述。
- 后续 TestFlight 或 App Store 审核说明不得要求测试常用出生资料入口。
- 当前真实可测试能力为首页查询、结果页分享、历史记录、历史详情分享、知识库收藏 / 最近阅读、本地数据管理、隐私政策、免责声明和开源许可。
