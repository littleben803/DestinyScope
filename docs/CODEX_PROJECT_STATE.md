...
- 阶段 6：历史记录基础回看已完成。
- 历史列表按查询时间倒序展示，历史详情页当前只保留称骨诗文、命格详解和系统分享入口。
- 历史记录收藏、置顶和填入首页重新查询入口已按后续产品优化移除。
- 阶段 7：首页信息架构优化已完成。
- 首页已拆分为定位说明、本地隐私提示、常用资料、输入卡片、最近历史摘要和知识库入口。
- 常用资料只填入首页输入，不自动查询、不自动保存历史、不重新计算。
- 最近历史摘要只读取本机轻量历史记录，不调用模型、不请求网络。
- 阶段 8：本地数据管理入口已完成。
- 设置页新增“本地数据管理”，可查看历史记录、常用资料、知识库收藏、最近阅读和首次使用说明状态。
- 支持清理历史记录、常用出生资料、知识库收藏、知识库最近阅读、首次使用说明状态和全部本地用户数据。
...
- 结果页结构已调整为：命格、称骨诗、权重明细、命格详解、复制与分享、免责声明。
